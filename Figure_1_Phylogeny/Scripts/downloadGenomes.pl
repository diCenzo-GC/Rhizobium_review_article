...
while(<>) {
	...
	push(@ftp,@line[3]); # make an array of the ftp links
...
foreach $species (@species) { # for each species of interest
	...
	@ftp2 = split('/',$ftp); # prepare a variable with the current file name
	system("mv @ftp2[10] Proteomes/$species"); # move the file to the correct directory and rename
	shift(@ftp); # shift the array to the next FTP link
}

#unzip the files
system("gunzip Proteomes/*.faa.gz");
