...
# List of files
$strainFile = 'intermediaryFiles/refseqGenomeInformation.txt';
...
while(<$strainList>) {
	chomp;
	@strainLine = split("\t",$_);
	@strainLine2 = split('_',@strainLine[4]);
	$partialStrain = "@strainLine2[0]_@strainLine2[1]";
	push(@strainsFull,@strainLine[4]);
	push(@strainsPartial,$partialStrain);
	push(@species,@strainLine[0]);
}

# Determine presence of the six genes
say("Species\tStrain\tNodA\tNodB\tNodC\tNifH\tNifD\tNifK\tNodABC\tNifHDK\tAll");
$count = -1;
foreach $i (@strainsFull) {
	...
	say("@species[$count]\t@strainsFull[$count]\t@hitArray[0]\t@hitArray[1]\t@hitArray[2]\t@hitArray[3]\t@hitArray[4]\t@hitArray[5]\t@hitArray[6]\t@hitArray[7]\t@hitArray[8]");
}
